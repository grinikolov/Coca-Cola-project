    ...
    public class CreateAsync_Should
    {
        ...
        public void UndeletesBar_WhenBarExist()
        {
            ...
            using (var context = new BCcontext(options))
            {
                ...
                var sut = new BarsService(context, mockMapper.Object, http.Object, coctailMapper.Object);
                var result = sut.CreateAsync(bar).Result;

                var dbResult = context.Bars.FirstOrDefaultAsync(x => x.Name == bar.Name).Result;
        ...
        {
            //Arrange
            var options = Utils.GetOptions("UndeletesBar_WhenBarExistAsync");
